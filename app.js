...
/* ============================================================
   ŞİRKET KURMA FORMU (dinamik olarak #startScreen içine yazılır —
   hem bulut hem yerel-yedek akış tarafından kullanılıyor)
   ============================================================ */
function renderCompanyNameForm(onSubmit) {
  ...
  screen.innerHTML = `
    <div class="start-card">
      ...
      <h1>${L.startTitle}</h1>
      <p>${L.startDesc}</p>
      <input id="nameInput" maxlength="24" placeholder="${L.namePlaceholder}" autocomplete="off">
      <button id="startBtn" disabled>${L.startBtn}</button>
    </div>`;
  screen.style.display = 'flex';
  const nameEl = document.getElementById('nameInput');
  const btnEl = document.getElementById('startBtn');
  nameEl.addEventListener('input', () => { btnEl.disabled = nameEl.value.trim().length === 0; });
  nameEl.addEventListener('keydown', e => { if (e.key === 'Enter' && !btnEl.disabled) btnEl.click(); });
  btnEl.addEventListener('click', () => onSubmit(nameEl.value.trim()));
}
function enterGame() {
...
function buyFloor(idx, cost) {
  ...
  if (supabaseUser) {
    insertKat(supabaseUser.id, idx).then(katId => { state.floors[idx - 1]._katId = katId; });
  }
  renderAll();
...
function openHirePopup(floorIdx, slotIdx) {
  ...
  popupCard.querySelector('.buy-btn').addEventListener('click', () => {
    ...
    if (supabaseUser && floor._katId) upsertCizer(floor._katId, slotIdx, slot);
...
function purchaseMarketItem(id) {
  nativePurchase('market_' + id, (success) => {
    ...
    if (supabaseUser) upsertVipOzellik(supabaseUser.id, id, { sahip: true, acik: true });
    saveGame();
    openMarketPopup();
...
function toggleMarketItem(id) {
  ...
  if (supabaseUser) upsertVipOzellik(supabaseUser.id, id, { sahip: state.market[id].owned, acik: state.market[id].on });
  saveGame();
  openMarketPopup();
  updateIconButtons();
}
...
function doRebirth() {
  ...
  saveGame(); renderAll(); updateIconButtons(); updateAdBuffWidget();
  if (supabaseUser) {
    upsertPara(supabaseUser.id, state);
    resetFloorsCloud(supabaseUser.id).then(katId => { if (state.floors[0]) state.floors[0]._katId = katId; });
  }
...
/* ============================================================
   BAŞLATMA AKIŞI: Dil → (Supabase varsa) E-posta/Kod Girişi → Oyun
   Supabase yapılandırılmamışsa (config.js/CDN yüklenemezse) otomatik
   olarak eski localStorage moduna düşer, oyun yine de açılır.
   ============================================================ */
...
async function bootCloud(user) {
  supabaseUser = user;
  const cloudState = await loadCloudState(user.id, freshState());
  if (cloudState && cloudState.companyName) {
    // Bu e-postaya kayıtlı bir şirket zaten var -> direkt oyuna gir
    state = cloudState;
    if (!Array.isArray(state.milestonesSeen)) state.milestonesSeen = [];
    if (!Array.isArray(state.realPrizeCodes)) state.realPrizeCodes = [];
    enterGame();
    ...
    // Yeni kullanıcı -> şirket kurma ekranı
    state = cloudState || freshState();
    renderCompanyNameForm(async (name) => {
      state.companyName = name;
      enterGame();
      queueDialogue(L.dlgWelcome1(state.companyName));
      queueDialogue(L.dlgWelcome2);
      advanceDialogue();
      saveGame();
      await upsertPara(supabaseUser.id, state);
    });
  }
}

function saveGameCloud() {
  saveGame(); // yerel yedek (çevrimdışı güvence)
  if (supabaseUser) {
    upsertPara(supabaseUser.id, state);
    const topFloor = state.floors[state.floors.length - 1];
    if (topFloor && topFloor._katId) {
      topFloor.slots.forEach((slot, i) => upsertCizer(topFloor._katId, i, slot));
    }
  }
}

document.addEventListener('visibilitychange', () => { if (document.hidden) saveGameCloud(); });
window.addEventListener('pagehide', saveGameCloud);
setInterval(saveGameCloud, 20000);

function startWhenReady() {
  initLanguage(() => {
    if (window.supabaseClient) initAuth(bootCloud);
...
if (window.supabaseClient) {
  ...
  window.addEventListener('supabase-ready', startWhenReady, { once: true });
  setTimeout(() => { if (!window.supabaseClient) startWhenReady(); }, 3000);
}
